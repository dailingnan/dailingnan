...
<script>
		$('#dg').datagrid({
	     url : '/move/discuss/findMoveDiscuss.do', 
	    columns:[[
			{field:'user',title:'留言用户',width:150,formatter:function(value,rowData,rowIndex){  
					return rowData.user.username;  
			}},
			{field:'cinemaBean',title:'电影院名称',width:150,formatter:function(value,rowData,rowIndex){  
					return rowData.cinemaBean.name;  
			}},
				...
				buttons:[{
					...
					handler:function(){
						var row = $('#dg').datagrid('getSelected');
						 if(confirm("你确定删除该条电影排片记录吗？")) {
						            //如果是true ，那么就把页面转向thcjp.cnblogs.com
						        location.href="/move/discuss/deleteMoveDiscuss.html?discussid="+row.discussid;
						     }
						    else{
						        //否则说明下了，赫赫
